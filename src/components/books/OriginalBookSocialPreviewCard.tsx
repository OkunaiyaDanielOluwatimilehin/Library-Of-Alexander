...
import { OriginalBook } from "../../hooks/useOriginalBooks";
import { Copy, Check, Sparkles, BookOpen, Layers } from "lucide-react";

interface OriginalBookSocialPreviewCardProps {
  book: OriginalBook;
}

export function OriginalBookSocialPreviewCard({ book }: OriginalBookSocialPreviewCardProps): React.JSX.Element {
  const [copied, setCopied] = useState(false);
  const shareUrl = `${window.location.origin}/original-book/${book.slug || book.id}`;
  ...
  const getExcerpt = (text: string) => {
    if (!text) return "Read original book and chapters online.";
        ...
        <div className="bg-white border border-[#e2e8f0] rounded-none overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col md:flex-row">
          ...
          <div className="p-4 space-y-1 bg-white flex-1 flex flex-col justify-center">
            <span className="text-[11px] font-mono text-stone-400 block tracking-tight lowercase truncate">
              {currentHost} › original-book › {book.slug || book.id}
            </span>
            <h1 className="font-sans font-extrabold text-sm sm:text-base text-stone-800 leading-tight">
              {book.title} | Original Publication on ReadersHub
            </h1>
            <p className="font-sans text-stone-700 text-sm leading-relaxed max-w-2xl text-justify mt-1 line-clamp-3">
              {book.synopsis}
            </p>
            <div className="flex flex-wrap items-center gap-2 pt-2">
              <span className="text-[10px] font-mono text-orange-700 font-extrabold bg-orange-50 px-2 py-0.5 rounded-none border border-orange-200 flex items-center gap-1">
                <BookOpen className="w-3.5 h-3.5 text-orange-500" /> {book.chapters?.length || 0} Chapters
              </span>
              <span className="text-[10px] font-mono text-stone-500 font-bold bg-stone-100 px-2 py-0.5 rounded-none border border-stone-200 flex items-center gap-1">
                <Layers className="w-3.5 h-3.5 text-stone-400" /> {book.genre}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-1.5 pt-1">
          ...
          <div className="flex bg-white border border-stone-250 font-mono text-xs text-stone-800 rounded-none overflow-hidden shadow-xs">
            <input 
              ...
              readOnly
              value={shareUrl}
              className="p-2.5 truncate flex-1 text-stone-500 select-all outline-none bg-stone-50/50 w-full"
            />
          </div>
        </div>
      </div>
...
export default OriginalBookSocialPreviewCard;
